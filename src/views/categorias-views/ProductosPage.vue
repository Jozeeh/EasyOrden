<template>
    <ion-page>
        ...
        <ion-content class="fondo">
            ...
            <ion-modal ref="modal" trigger="open-modal" :enter-animation="enterAnimation" :leave-animation="leaveAnimation" :is-open="modalInfo">
                <ion-content class="contenido-info">
                    ...
                    <ion-card-content>
                        <ion-card-subtitle>{{ modalInfoProducto.categoria }} (ID: {{ modalInfoProducto.idPlato }})</ion-card-subtitle>
                        <h1>{{ modalInfoProducto.nombrePlato }}</h1>
                        <h1>${{ modalInfoProducto.precio }}</h1>
                        Lorem ipsum dolor sit amet consectetur adipisicing elit. Maiores odio amet fuga dolor illo, aut reiciendis. Placeat assumenda, odit officia natus saepe ut rem fugiat fuga inventore, distinctio facere cumque.
                        Modi, quas pariatur. Optio unde repellendus deleniti, ut, reprehenderit, cumque tempora dolore exercitationem neque odit vero nihil dicta officiis animi. Nesciunt suscipit voluptatum labore repellendus reiciendis autem expedita id quibusdam?
                    </ion-card-content>
                </ion-content>

                <ion-toolbar style="background-color: white;">
                    <ion-button color="danger" slot="end" @click="modalInfo=false"><ion-icon aria-hidden="true" slot="start" :ios="close" :md="close"></ion-icon>Cerrar</ion-button>
                    <ion-button color="success" slot="end" @click="agregarCarrito(modalInfoProducto)"><ion-icon aria-hidden="true" slot="start" :ios="cart" :md="cart"></ion-icon>Agregar</ion-button>
                </ion-toolbar>
            </ion-modal>
